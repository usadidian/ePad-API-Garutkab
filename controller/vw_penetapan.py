...
class vw_penetapan(db.Model, SerializerMixin):
    __tablename__ = 'vw_penetapan'
    ...
    FaxKecamatan = db.Column(db.String, nullable=True)
    ...
    class ListAll(Resource):
        ...
        def get(self, *args, **kwargs):
            ...
            print(UserId)
            select_query = vw_penetapan.query

            # SEARCH
            if args['search'] and args['search'] != 'null':
                search = '%{0}%'.format(args['search'])
                select_query = select_query.filter(
                    or_(vw_penetapan.KohirID.ilike(search),
                        vw_penetapan.TglPenetapan.ilike(search))
                )
            ...
            if args['sort']:
                ...
                select_query = select_query.order_by(vw_penetapan.KohirID)
