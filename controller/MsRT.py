...
class MsRT(db.Model, SerializerMixin):
    __tablename__ = 'MsRT'
    RTID = db.Column(db.Integer, primary_key=True)
    RT = db.Column(db.String, nullable=False)
    UserUpd = db.Column(db.String, nullable=False)
    DateUpd = db.Column(db.String, nullable=False)

    RWID = db.Column(db.Integer, db.ForeignKey('MsRW.RWID'), nullable=False)
    ...
    class ListAll1(Resource):
        def get(self, *args,  **kwargs):
            ...
            select_query = db.session.execute(
                f"SELECT RTID, 'RT.' + RT AS RT FROM MsRT ORDER BY RT ")
            result=[]
    ...
    class ListAll2(Resource):
        def get(self, rwid, *args,  **kwargs):
            parser = reqparse.RequestParser()
            parser.add_argument('KecamatanID', type=str)
            parser.add_argument('KelurahanID', type=str)
            select_query = db.session.execute(
                f"SELECT RTID, 'RT.' + RT AS RT FROM MsRT WHERE RWID='{rwid}' ORDER BY RT ")
            result=[]
            for row in select_query:
                d = {}
                for key in row.keys():
    ...
    class ListAll(Resource):
        ...
        def get(self, *args, **kwargs):
            ...
            print(UserId)
            select_query = MsRT.query

            # SEARCH
            if args['search'] and args['search'] != 'null':
                search = '%{0}%'.format(args['search'])
                select_query = select_query.filter(
                    or_(MsRT.RTID.ilike(search),
                        MsRT.RT.ilike(search))
                )

            # SORT
            if args['sort']:
                if args['sort_dir'] == "desc":
                    sort = getattr(MsRT, args['sort']).desc()
                else:
                    sort = getattr(MsRT, args['sort']).asc()
                select_query = select_query.order_by(sort)
            else:
                select_query = select_query.order_by(MsRT.RTID)
        ...
        def post(self, *args, **kwargs):
            parser = reqparse.RequestParser()
            # parser.add_argument('RTID', type=str)
    ...
    class ListById(Resource):
        ...
        def get(self, id, *args, **kwargs):
            try:
                select_query = MsRT.query.filter_by(RTID=id).first()
        ...
        def put(self, id, *args, **kwargs):
            ...
            try:
                select_query = MsRT.query.filter_by(RTID=id).first()
                if select_query:
                    # select_query.RTID = args['RTID']
                    select_query.RWID = args['RWID']
                    select_query.RT = args['RT']
        ...
        def delete(self, id, *args, **kwargs):
            try:
                delete_record = MsRT.query.filter_by(RTID=id)
