...
class vw_penetapanomzet(db.Model, SerializerMixin):
    __tablename__ = 'vw_penetapanomzet'
    ...
    class ListAll(Resource):
        ...
        def get(self, *args, **kwargs):
            ...
            print(UserId)
            select_query = vw_penetapanomzet.query

            # SEARCH
            if args['search'] and args['search'] != 'null':
                search = '%{0}%'.format(args['search'])
                select_query = select_query.filter(
                    or_(vw_penetapanomzet.KohirID.ilike(search),
                        vw_penetapanomzet.TglPenetapan.ilike(search))
                )

            # SORT
            if args['sort']:
                if args['sort_dir'] == "desc":
                    sort = getattr(vw_penetapanomzet, args['sort']).desc()
                else:
                    sort = getattr(vw_penetapanomzet, args['sort']).asc()
                select_query = select_query.order_by(sort)
            else:
                select_query = select_query.order_by(vw_penetapanomzet.KohirID)
