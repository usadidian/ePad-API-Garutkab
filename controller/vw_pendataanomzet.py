...
class vw_pendataanomzet(db.Model, SerializerMixin):
    __tablename__ = 'vw_pendataanomzet'
    ...
    class ListAll(Resource):
        ...
        def get(self, *args, **kwargs):
            ...
            print(UserId)
            select_query = vw_pendataanomzet.query

            # SEARCH
            if args['search'] and args['search'] != 'null':
                search = '%{0}%'.format(args['search'])
                select_query = select_query.filter(
                    or_(vw_pendataanomzet.PendataanID.ilike(search),
                        vw_pendataanomzet.TglPendataan.ilike(search))
                )

            # SORT
            if args['sort']:
                if args['sort_dir'] == "desc":
                    sort = getattr(vw_pendataanomzet, args['sort']).desc()
                else:
                    sort = getattr(vw_pendataanomzet, args['sort']).asc()
                select_query = select_query.order_by(sort)
            else:
                select_query = select_query.order_by(vw_pendataanomzet.DateUpd)
