...
class vw_jenistarif(db.Model, SerializerMixin):
    __tablename__ = 'vw_jenistarif'
    ...
    Periode = db.Column( db.String, nullable=False )
    ...
    class ListAll(Resource):
        ...
        def get(self, *args, **kwargs):
            ...
            print(UserId)
            select_query = vw_jenistarif.query

            # SEARCH
            if args['search'] and args['search'] != 'null':
                search = '%{0}%'.format(args['search'])
                select_query = select_query.filter(
                    or_(vw_jenistarif.NamaLokasi.ilike(search),
                        vw_jenistarif.Nama.ilike(search))
                )
            ...
            if args['sort']:
                ...
                select_query = select_query.order_by(vw_jenistarif.NamaLokasi)
