...
class TaskUploadOPDAvatar(Resource):
    def post(self, *args, **kwargs):
        ...
        cloudinary_floder = f"OPD_Avatar/{origin}avatar_opd_{str(id)}"
        if data_par['self_upload']:
            select_query = MsOPD.query.filter_by(OPDID=id).first()
            cloudinary_floder = f"OPD_Avatar/{origin}avatar_opd_{str(id)}_self_uploaded"
        else:
            select_query = MsOPD.query.filter_by(OPDID=id).first()
        try:
            ...
            notification_data = {
                "notification": {
                    "title": appName,
                    "body": f"Foto usaha {select_query.NamaOPD} sedang di unggah",
                    "priority": "high",
                    "icon": appFrontWebLogo,
                    "click_action": f"{appFrontWebUrl()}#/admin/pendaftaranobjekpajak",
                },
                "data": {
                    "type": "avatar_upload_start",
                    "page": "pendaftaranobjekpajak",
                    "id": select_query.OPDID
                }
            }
                ...
                if len(filenames) > 1:
                    ...
                    if check_image_nudity_output['nudity']['safe'] < 0.50:
                        notification_data = {
                            "notification": {
                                "title": appName,
                                "body": f"Foto usaha (wajib pajak) {select_query.NamaOPD} gagal di unggah karena mengandung unsur pornografi atau kekerasan",
                                "priority": "high",
                                "icon": appFrontWebLogo,
                                "click_action": f"{appFrontWebUrl()}#/admin/pendaftaranobjekpajak",
                            },
            ...
            notification_data = {
                "notification": {
                    "title": appName,
                    "body": f"Foto usaha {select_query.NamaOPD} berhasil di unggah",
                    "priority": "high",
            ...
            notification_data = {
                "notification": {
                    "title": appName,
                    "body": f"Foto usaha {select_query.NamaOPD} gagal di unggah",
                    "priority": "high",
                    "icon": appFrontWebLogo,
                    "click_action": f"{appFrontWebUrl()}#/admin/pendaftaranobjekpajak",
                },
...
class TaskDeleteOPDAvatar(Resource):
    def post(self, *args, **kwargs):
        data_par = json.loads(request.data.decode())
        path = data_par['path']
        ...
        try:
            ...
            origin = request.headers.get('Origin')
            origin = origin.replace('http://', '').replace('https://', '') + '/' if origin else ''
            delete_img = cloudinary.uploader.destroy('OPD_Avatar/' + origin + get_public_id)
