...
class TaskUploadWpAvatar(Resource):
    def post(self, *args, **kwargs):
        ...
        cloudinary_floder = f"WP_Avatar/{origin}avatar_wp_{str(id)}"
        if data_par['self_upload']:
            select_query = MsWPOData.query.filter_by(WPID=id).first()
            cloudinary_floder = f"WP_Avatar/{origin}avatar_wp_{str(id)}_self_uploaded"
        else:
        ...
        try:
            ...
            notification_data = {
                "notification": {
                    ...
                    "click_action": f"{appFrontWebUrl()}#/admin/pendaftaran",
                },
                "data": {
                    "type": "avatar_upload_start",
                    "page": "pendaftaran",
                    "id": select_query.WPID
                }
            }
                ...
                if len(filenames) > 1:
                    ...
                    if check_image_nudity_output['nudity']['safe'] < 0.50:
                        notification_data = {
                            "notification": {
                                "title": appName,
                                "body": f"Foto usaha (wajib pajak) {select_query.NamaBadan} gagal di unggah karena mengandung unsur pornografi atau kekerasan",
                                "priority": "high",
                                "icon": appFrontWebLogo,
                                "click_action": f"{appFrontWebUrl()}#/admin/pendaftaran",
                            },
...
class TaskDeleteWpAvatar(Resource):
    def post(self, *args, **kwargs):
        data_par = json.loads(request.data.decode())
        path = data_par['path']
        ...
        try:
            ...
            origin = request.headers.get('Origin')
            origin = origin.replace('http://', '').replace('https://', '') + '/' if origin else ''
            delete_img = cloudinary.uploader.destroy('WP_Avatar/' + origin + get_public_id)
