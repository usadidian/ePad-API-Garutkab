    ...
    def post(self, *args, **kwargs):
        ...
        try:
            ...
            notification_data = {
                "notification": {
                    "title": appName,
                    "body": f"Foto Profil {select_query.nama_user} sedang di unggah",
                    "priority": "high",
                ...
                if len(filenames) > 1:
                    ...
                    if check_image_nudity_output['nudity']['safe'] < 0.50:
                        notification_data = {
                            "notification": {
                                "title": appName,
                                "body": f"Foto Profil {select_query.nama_user} gagal di unggah karena mengandung unsur pornografi atau kekerasan",
                                "priority": "high",
                                "icon": appFrontWebLogo,
                                "click_action": f"{appFrontWebUrl()}#/admin",
                            },
            ...
            for row in filenames:
                ...
                upload_result = cloudinary.uploader.upload(f'{folder_name}/{row}',
                                                           public_id=f"Avatar/{origin}avatar_{str(id)}")
            ...
            notification_data = {
                "notification": {
                    "title": appName,
                    "body": f"Foto Profil {select_query.nama_user} berhasil di unggah",
                    "priority": "high",
            ...
            notification_data = {
                "notification": {
                    "title": appName,
                    "body": f"Foto profil {select_query.nama_user} gagal di unggah",
                    "priority": "high",
                    "icon": appFrontWebLogo,
                    "click_action": f"{appFrontWebUrl()}#/admin",
                },
...
class TaskDeleteAvatar(Resource):
    def post(self, *args, **kwargs):
        data_par = json.loads(request.data.decode())
        path = data_par['path']
        ...
        try:
            ...
            origin = request.headers.get('Origin')
            origin = origin.replace('http://', '').replace('https://', '') + '/' if origin else ''
            delete_img = cloudinary.uploader.destroy('Avatar/' + origin + get_public_id)
            delete_img.delete()
            db.session.commit()
