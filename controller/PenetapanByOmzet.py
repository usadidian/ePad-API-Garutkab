...
class PenetapanByOmzet(db.Model, SerializerMixin):
    __tablename__ = 'PenetapanByOmzet'
    ...
    class ListAll(Resource):
        ...
        def get(self, *args, **kwargs):
            ...
            print(UserId)
            select_query = PenetapanByOmzet.query

            # SEARCH
            if args['search'] and args['search'] != 'null':
                search = '%{0}%'.format(args['search'])
                select_query = select_query.filter(
                    or_(PenetapanByOmzet.KohirID.ilike(search),
                        PenetapanByOmzet.TglPenetapan.ilike(search))
                )

            # SORT
            if args['sort']:
                if args['sort_dir'] == "desc":
                    sort = getattr(PenetapanByOmzet, args['sort']).desc()
                else:
                    sort = getattr(PenetapanByOmzet, args['sort']).asc()
                select_query = select_query.order_by(sort)
            else:
                select_query = select_query.order_by(PenetapanByOmzet.KohirID)
    ...
    class ListById(Resource):
        ...
        def get(self, id, *args, **kwargs):
            try:
                select_query = PenetapanByOmzet.query.filter_by(KohirID=id).first()
        ...
        def delete(self, id, *args, **kwargs):
            try:
                delete_record = PenetapanByOmzet.query.filter_by(KohirID=id)
