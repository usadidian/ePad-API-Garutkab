...
class Pembayaran(db.Model, SerializerMixin):
    __tablename__ = 'Pembayaran'
    PembayaranID = db.Column( db.Integer, primary_key=True )
    WPID = db.Column(db.Integer, nullable=True)
    OPDID = db.Column(db.Integer, nullable=True)
    ...
    PajakOpsen = db.Column(db.Numeric(precision=8, asdecimal=False, decimal_return_scale=None), nullable=True)
    ...
    Saldo = db.Column(db.Numeric(precision=8, asdecimal=False, decimal_return_scale=None), nullable=True)
    ...
    strjml = db.Column( db.String, nullable=True )
    decjml = db.Column( db.Numeric( precision=8, asdecimal=False, decimal_return_scale=None ), nullable=True )
    ...
    class ListAll(Resource):
            ...
            def get(self, *args, **kwargs):
                ...
                print(UserId)
                select_query = Pembayaran.query

                # SEARCH
                if args['search'] and args['search'] != 'null':
                    search = '%{0}%'.format(args['search'])
                    select_query = select_query.filter(
                        or_(Pembayaran.KohirID.ilike(search),
                            Pembayaran.TglPenetapan.ilike(search))
                    )
                ...
                if args['sort']:
                    ...
                    select_query = select_query.order_by(Pembayaran.KohirID)
