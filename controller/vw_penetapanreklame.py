...
class vw_penetapanreklame(db.Model, SerializerMixin):
    __tablename__ = 'vw_penetapanreklame'
    ...
    Aktif = db.Column(db.String, nullable=False)
    ...
    class ListAll(Resource):
        ...
        def get(self, *args, **kwargs):
            ...
            print(UserId)
            select_query = vw_penetapanreklame.query

            # SEARCH
            if args['search'] and args['search'] != 'null':
                search = '%{0}%'.format(args['search'])
                select_query = select_query.filter(
                    or_(vw_penetapanreklame.KohirID.ilike(search),
                        vw_penetapanreklame.TglPenetapan.ilike(search))
                )

            # SORT
            if args['sort']:
                if args['sort_dir'] == "desc":
                    sort = getattr(vw_penetapanreklame, args['sort']).desc()
                else:
                    sort = getattr(vw_penetapanreklame, args['sort']).asc()
                select_query = select_query.order_by(sort)
            else:
                select_query = select_query.order_by(vw_penetapanreklame.KohirID)
