...
class MsAirHdr(db.Model, SerializerMixin):
    __tablename__ = 'MsAirHdr'
    ATGrup = db.Column(db.Integer, primary_key=True)
        ...
        def get(self, *args, **kwargs):
            try:
                select_query = db.session.execute(
                    f"SELECT Nama + Nilai + convert(varchar,Nama) AS Kode,Nama,"
                    f"right('000'+convert(varchar,Nama),3) + ' - ' + ATGrup + ', ' + Kecamatan AS Nama FROM MsAirHdr j "
                    f"LEFT JOIN MsKecamatan c ON j.ATUrut = c.ATUrut ORDER BY ATGrup ")
                result=[]
    ...
    class ListAll(Resource):
        ...
        def get(self, *args, **kwargs):
            ...
            select_query = db.session.query(MsAirHdr.ATGrup, MsAirHdr.Nama)
            # SEARCH
            if args['search'] and args['search'] != 'null':
                search = '%{0}%'.format(args['search'])
                select_query = select_query.filter(
                    or_(MsAirHdr.Nama.ilike(search),
                        MsAirHdr.ATGrup.ilike(search))
                )

            # SORT
            if args['sort']:
                if args['sort_dir'] == "desc":
                    sort = getattr(MsAirHdr, args['sort']).desc()
                else:
                    sort = getattr(MsAirHdr, args['sort']).asc()
                select_query = select_query.order_by(sort)
            else:
                select_query = select_query.order_by(MsAirHdr.ATGrup)
        ...
        def post(self, *args, **kwargs):
            ...
            select_query = db.session.execute(
                f"SELECT CAST(MAX(CAST(Nama AS int) + 1) AS varchar(10)) AS NextID FROM MsAirHdr")
    ...
    class ListById(Resource):
        ...
        def get(self, id, *args, **kwargs):
            try:
                select_query = MsAirHdr.query.filter_by(ATGrup=id).first()
        ...
        def delete(self, id, *args, **kwargs):
            try:
                delete_record = MsAirHdr.query.filter_by(ATGrup=id)
